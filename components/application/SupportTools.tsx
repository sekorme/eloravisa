...
export default function SupportTools() {
    return (
        <Card className="p-8 space-y-8 bg-white dark:bg-slate-950 border-none shadow-2xl rounded-3xl relative overflow-hidden group">
            <div className="absolute -bottom-24 -left-24 w-64 h-64 bg-primary/10 rounded-full blur-3xl transition-transform duration-1000 group-hover:scale-125" />
            ...
            <div className="relative z-10 space-y-6">
                <div>
                    ...
                    <p className="text-sm font-medium text-slate-500 dark:text-slate-400">
                        Unlock high-performance <span className="text-primary font-bold">AI guidance</span> to secure your visa.
                    </p>
                </div>
                ...
                <div className="grid gap-4">
                    {[
                        ...
                        { label: "Telegram Guidance", icon: Send, color: "text-primary bg-primary/10 dark:text-primary border-primary/20", href: "#" },
                    ].map((tool) => (
                        <Link href={tool.href} key={tool.label} className="block group/item">
                            <div className="flex items-center justify-between w-full h-16 px-5 rounded-2xl bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/10 transition-all group-hover/item:border-primary/50 group-hover/item:bg-white dark:group-hover/item:bg-white/10 group-hover/item:shadow-xl group-hover/item:shadow-primary/10 group-hover/item:-translate-y-1">
                                ...
                                <ArrowRight size={18} className="text-slate-300 group-hover/item:text-primary group-hover/item:translate-x-1 transition-all" />
                            </div>
                        </Link>
                    ))}
                </div>
