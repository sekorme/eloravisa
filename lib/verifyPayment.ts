...
const getDurationInDays = (plan: string): number => {
    ...
    if (plan.toLowerCase().includes('6 month')) return 180;
 ...
 */
export async function verifyPayment({ reference, userId, email,name,phone,  plan }: any) {
    ...
    if (data.status === 'success') {
        ...
        await addDoc(collection(db, 'payments'), {
            ...
            phone, // Convert amount from kobo to naira
            plan,
            status: data.status,
            name,
            reference,
